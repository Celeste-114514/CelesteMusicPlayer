...
- 🐛 修复
  - 🔘 **开机自启误自启**：此前在设置里关掉开机自启后仍会自启（残留注册表清不掉）。现每次启动按设置值幂等校正注册表，关了即彻底清除，且只动本程序自己的启动项
  - 🔊 **音量重启回退 80% 进一步根治**：音量滑条默认值改为 0（避免初始化即误写盘）+ 新增启动回填闸门，回填完成前任何音量变化都不落盘
  - 🎚️ **多声道音源播放失败修复**：共享模式转码固定输出立体声，并在播放前新增「多声道→立体声降混」兜底，根治 `Value does not fall within the expected range`
- 🛠️ 优化使用体验
  - ▶️ 「播放所有歌曲」更聪明：只播当前界面**实际显示**的这一批（某搜索结果 / 某歌手 / 某流派），在「播放队列」界面则变为「播该队列」且保持顺序
  - 🖼️ 专辑封面居中（横版封面不再靠左）；艺术家详情页专辑墙排序状态保留，来回切换不再重置
  - 📚 手动「加入音乐库」持久化（重启 / 刷新曲库不丢），按钮带「已加入」确认态
  - 🎨 封面背景与界面外观、启动流程与单实例调度小幅调整
