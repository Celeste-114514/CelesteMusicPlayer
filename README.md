...
- 🐛 修复
  - 📥 **修复应用内更新「下载失败：网络中断或下载地址失效」**：进度条走到 100% 后必报此错，真因与网络无关 —— 下载用的文件句柄被程序自己独占锁住，紧接着的完整性校验打不开刚下载的安装包而失败，且旧版把任何异常都一律当成网络问题提示。现改为下载完立即释放文件锁、校验改为边下载边计算指纹，出错提示按真实原因分开说明（网络中断 / 连接超时 / 文件被占用 / 目录无权限）
...
### v26.9.20（2026-09-21）
- ✨ 新增功能
  - 🖱️ **多选支持 Shift 连选**：曲库 / 播放列表等多选界面，Shift+单击即可一次选中一段歌曲（Ctrl+单击跳选延续原有操作）
  - ⚙️ **启动后进入哪个分类**：设置新增启动落地页选项，打开程序直接到常用分类
  - 🔐 **更新下载 SHA-256 校验（P0-1）**：应用内更新会按当前已安装的变体（框架依赖 / 自包含）自动下载对应安装包，下载完成后先校验哈希再安装；关于面板会提示即将下载哪一种包
- 🔊 改进
  - 📦 **发布流程全自动**：推送版本 tag 后，CI 自动完成构建 → EQ 回归测试 → 打包（框架依赖 + 自包含双安装包）→ 发布到 GitHub Release（附 SHA256SUMS.txt），不再需要手动打包上传
- 🐛 修复
  - 💾 **5 个设置项重启归零**：交叉淡化时长、SRC 目标采样率 / 质量 / Dither、歌词偏移此前保存后重启即恢复默认值，已修复
  - 🎨 修复浅色主题下列表选中项文字变白看不清的问题
  - 🖱️ 修复多选组件（Shift 连选）的列表内存泄漏与标志重入问题
  - 📦 修复自包含版发布时更新助手构建失败的问题（NETSDK1150）
- 🔧 开发
  - 打包流程参数化：安装包不再硬编码路径 / 版本号，新增一键双包脚本
  - CI 修复：PowerShell 脚本补 UTF-8 BOM（修中文乱码）、restore 分工程指定 RID（修 NETSDK1047）、打包前自动安装 NSIS（修运行器缺 makensis）
  - 清理：共享 HttpClient，消除短命实例（P1-6）

### v26.9.19.1（2026-09-19）
- 🐛 修复
  - 🔄 **应用内更新覆盖失败**：「下载更新」完成后改为优先交给 `CelesteUpdater` 更新助手执行（等主程序真正退出再跑安装包，避免安装时撞上被占用的程序文件导致覆盖失败、重开仍是旧版本）；本机找不到助手时才退回原有 cmd 方式
- 🔧 开发
  - 🧪 新增免发版本地验证开关：放置 `%LOCALAPPDATA%\CelesteMusicPlayer\dev-update.txt`（第一行假版本号、第二行本地安装包路径）即可完整走一遍「检查→下载→助手接管→重启→删包」链路；文件不存在时对正常使用零影响

### v26.9.19（2026-09-19）
- ✨ 新增功能
  - ☁️ **WebDAV 网络音乐库**：设置新增「网络音乐库」页，可接入 PikPak / 群晖 / 坚果云等 WebDAV 网盘；主界面左侧逐层浏览网盘目录、直接点播，边播边缓存（缓存目录与上限可配，密码本地加密保存）；支持跟随系统 / 不使用 / 手动代理三种模式
  - 📊 **播放信息页可视化多选一**：波形 / 频谱柱（方块段+峰值帽）/ 径向频谱（环绕封面圆环）/ 示波器 / 李萨如相位图 / 不显示，在设置中切换
  - 💻 **极客「终端」播放页**：极客界面下播放页变身为信号监控台（音频流信息 + 方块频谱 + 电平峰值 + 相位图 + 播放队列），底色近黑、强调色跟随主题
  - 🧭 **音效处理页信号链导航**：左栏「音效处理」新增处理链路模块导航（输入余量 → 采样率 → 参数 EQ → 耳机校正 → FIR → 声道 → ReplayGain → 输出安全监控），点选直达对应分区
  - 🔲 **极客浏览面板重塑**：极客模式下右侧歌曲 / 专辑 / 文件夹等列表全面直角化——表格细线、方形封面、`[MP3 · ALAC]` 式格式标签、行首序号补零对齐、搜索框 `> ` 命令行提示符；切回图景 / 经典界面可干净还原
- 🔊 改进
  - 📈 播放内核新增只读输出监控（会话峰值 / 削波计数、示波器取样），房间校正页显示卷积削波提示与 IR 信息、新增卷积余量微调；bit-perfect 输出字节流不受影响
  - 🎛️ 频谱分析器重写以配合新可视化样式
- ⚙️ 调整
  - 移除旧「查找歌曲」弹窗（功能已由曲库内搜索承接）
