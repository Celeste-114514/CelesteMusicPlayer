...
static IMMDevice* resolve_device(
    ...
    if (deviceId != NULL && deviceId[0] != L'\0') {
        for (size_t i = 0; i < devices.size(); ++i) {
            if (wcscmp(devices[i].id, deviceId) == 0) {
                selectedId = devices[i].id;
                break;
            }
        }
        if (selectedId == NULL) {
            set_error(error, errorLen, "Requested WASAPI device id not found", S_OK);
            return NULL;
        }
...
/* Celeste 接入改动：四舍五入（远离零）。
   原来用 C 的 (int) 强转 = 向零截断，配 8388607/32767/2147483647 的不对称系数，
   结果是系统性 ±1 LSB 误差：24bit 源 → float（精确）→ 设备 PCM24 每两个样本就有一个差 1，
   bit-perfect 不成立。改为 2^n 对称系数 + 四舍五入 + 钳位后，
   16/24/32bit 整数 ⇄ float32 的往返是精确可逆的（float32 24 位尾数 ≥ 24bit 范围）。 */
static inline int32_t round_sample(float value) {
    ...
    switch (kind) {
        ...
        case WASAPI_FORMAT_PCM24_IN_32: {
            ...
            for (uint32_t i = 0; i < total; ++i) {
                ...
                int32_t v = round_sample(s * 8388608.0f);
        ...
        case WASAPI_FORMAT_PCM32: {
            ...
            for (uint32_t i = 0; i < total; ++i) {
                float s = clamp_sample(input[i]);
                /* 用 double 中间量：float 尾数只有 24 位，s*2^31 直接算会丢低位 */
                double scaled = static_cast<double>(s) * 2147483648.0;
                int32_t v = scaled >= 0.0 ? static_cast<int32_t>(scaled + 0.5)
        ...
        default: {
            ...
            for (uint32_t i = 0; i < total; ++i) {
                float s = clamp_sample(input[i]);
                int32_t v = round_sample(s * 32768.0f);
                if (v > 32767) v = 32767;
...
static int wasapi_exclusive_start_impl(
    ...
    *outRuntime = NULL;
    ...
    device = resolve_device(devices, targetDeviceName, targetDeviceIndex, deviceId, error, errorLen);
    if (device == NULL) {
        com_scope_leave(&com);
        return -1;
    }
    ...
    if (SUCCEEDED(hr)) {
        if (runtime->renderMode == WASAPI_RENDER_PCM && runtime->callback != NULL) {
            /* Celeste 接入改动：用真实音频预填首个设备缓冲（feeder 已提前把货 push 进 ring），
               不再填静音——否则设备会把整整一个周期（默认 100ms，用户可能设到 300ms）的静音
               播完才轮到真实音频，每首歌开头都白聋一段。
               ring 尚未备货时 renderInterleaved 自然输出静音兜底，无越界风险。 */
            std::vector<float> primeScratch(
                static_cast<size_t>(runtime->bufferFrameCount) * runtime->channels, 0.0f);
            runtime->callback(runtime->userData, primeScratch.data(),
                              runtime->bufferFrameCount, runtime->channels);
            convert_float_to_endpoint(
                primeScratch.data(),
                endpointBuffer,
                runtime->bufferFrameCount,
                runtime->channels,
                runtime->format.kind);
            hr = runtime->renderClient->ReleaseBuffer(runtime->bufferFrameCount, 0);
        } else {
